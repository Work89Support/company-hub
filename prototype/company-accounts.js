...
function openAccountDuties(index){
 ...
 form.onsubmit=async e=>{e.preventDefault();if(!collect())return;const buttons=form.querySelectorAll('button');buttons.forEach(b=>b.disabled=true);try{const result=await accountCall('provision',{name:row.source_name,department_code:row.department_code,department_codes:row.department_codes,position_title:row.position_title});row.created=true;closeModal();renderAccountCandidates();if(result.existing)toast('มีบัญชีแล้ว ให้แก้หน้าที่จากบัญชีเดิม');else accountShowCode(result);}catch(error){form.querySelector('[role=alert]').textContent=error.message;buttons.forEach(b=>b.disabled=false);}};
}
...
async function createReviewedAccounts(){
 ...
 for(const row of rows){try{const result=await accountCall('provision',{name:row.source_name,department_code:row.department_code,department_codes:row.department_codes||[row.department_code],position_title:row.position_title||'พนักงาน'});if(result.initial_password){ACCOUNT_INITIAL_CODES.push(result);renderInitialAccountCodes();}row.created=true;done++;}catch(e){row.review_notes=e.message;row.review_status='review_identity';failed++;}status.textContent='ดำเนินการแล้ว '+(done+failed)+' / '+rows.length+' · สำเร็จ '+done+' · ต้องตรวจเพิ่ม '+failed;}
