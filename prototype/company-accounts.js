...
function liveAccountCandidates(activities,members){
 ...
 const rows=[...map.values()],normalize=s=>s.normalize('NFKC').toLowerCase().replace(/[\p{S}\p{Z}]/gu,'');
 return rows.filter(r=>!r.linked).map(r=>{const notes=[];if(['ทุกคน','all','hr','buki grace','บอส แก๋ม'].includes(r.source_name.toLowerCase())||r.source_name.includes('/'))notes.push('ชื่อกลุ่มหรือหลายคน');const variants=rows.filter(x=>x!==r&&x.department_code===r.department_code&&normalize(x.source_name)===normalize(r.source_name));if(variants.length)notes.push('ชื่อใกล้เคียง: '+variants.map(x=>x.source_name).join(', '));return {...r,review_status:notes.length?'review_identity':'ready',review_notes:notes.join(' · ')};}).sort((a,b)=>(a.department_code+a.source_name).localeCompare(b.department_code+b.source_name,'th'));
}
