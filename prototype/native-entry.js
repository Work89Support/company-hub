...
function entryMissing(r){
 ...
 if(!r.employee_id)m.push(r.employee_name?.trim()?'เชื่อมบัญชีเข้าใช้งานของ '+r.employee_name.trim():'บัญชีเข้าใช้งานของเจ้าของงาน');
...
function entryOwnerOptions(selected=''){
 ...
 select.innerHTML=`<option value="">${esc(entryOwnerPlaceholder(select.dataset.ownerName))}</option>`+ENTRY_PEOPLE.filter(p=>p.department_code===dept&&(manager||p.id===ACCESS_PROFILE.id)).map(p=>`<option value="${esc(p.id)}" ${p.id===selected?'selected':''}>${esc(p.display_name)}</option>`).join('');
...
async function saveActivityEntry(row,requestId){
 ...
 try{const r=await SB.rpc('save_activity_entry',{p_id:row?.id||null,p_expected_revision:row?.entry_revision??null,p_entry:v});if(r.error)throw r.error;closeModal();await loadActivities();if(VIEW==='completeness')RENDER.completeness();else if(VIEW==='activity')RENDER.activity();toast('บันทึกข้อมูลครบแล้ว');const saved=Array.isArray(r.data)?r.data[0]:r.data;if(saved?.id&&typeof openWorkKpis==='function')await openWorkKpis('activity',saved.id,saved.department_code);}
