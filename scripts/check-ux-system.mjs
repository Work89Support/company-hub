...
window.eval(source+`\nwindow.reviewAPI={checkWorkspace:async()=>{
 ...
 DATA_HEALTH.activity.state='loading';RENDER.dash();DATA_HEALTH.activity.state='ready';integrityRenderHealth();await Promise.resolve();if(main.querySelector('.ux-period-count b').textContent!=='1')throw new Error('Dashboard retained a stale loading count');
